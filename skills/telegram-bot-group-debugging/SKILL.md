...
9. **Schedule recurring HR draft templates as isolated script jobs.** For a recurring template that must be the only visible Telegram output, put template generation and outbox delivery in a Node script, schedule it with `sessionTarget: "isolated"` and `delivery.mode: "none"`, and use an IANA schedule timezone such as `Asia/Manila`. The script must reset only that workflow’s `draft`/`awaitingEdit` state, write the HR Drafts outbox payload with its existing Compose Draft callback, and invoke the existing sender. Add a `--dry-run` mode that returns the generated payload without writing state or sending. Verify the dry run for each workflow, then inspect the saved schedule; manual force-runs are available for an intentional live delivery test.
